...
/**
 * GameScreenLevel3 represents the third level of the game.
...
public class GameScreenLevel3 extends ScreenAdapter {
    private Game game;
    private Sound ding1;
    private Sound ding2;
    private Sound ding3;
    ...
    /**
     * Initializes the level with a reference to the main game object and the score from the previous level.
     *
     * @param game   The main game object managing the screens.
     * @param score2 The cumulative score from the previous level.
     * @param difficulty The difficulty of the save.
     */
    public GameScreenLevel3(Game game, String difficulty, int score2)
    {
        this.game = game;
        ...
        enemyIdleSheet = new Texture(Gdx.files.internal("assets/bossAttack2.png"));

        // split the spritesheets
        TextureRegion[][] tmp1 = TextureRegion.split(wizIdleSheet, wizIdleSheet.getWidth() / 6, wizIdleSheet.getHeight());
        TextureRegion[][] tmp2 = TextureRegion.split(enemyIdleSheet, enemyIdleSheet.getWidth() / 24, enemyIdleSheet.getHeight());
        ...
        TextureRegion[] banditFrames = new TextureRegion[24];
        index = 0;
        for (int i = 0; i < 1; i++) {
            for (int j = 0; j < 24; j++) {
    ...
    /**
     * Loads and sets up the initial state of the level when the screen is shown.
     */
    @Override
    public void show()
    {
        ...
        pauseButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                count--;
                game.setScreen(new PauseScreen(game, GameScreenLevel3.this));
            }
        });
    ...
    /**
     * Loads the words for the player to type from a JSON file.
     */
    private void loadWords()
    {
        ...
        try
        {
            ...
            if (difficulty.equals("easy"))
            {
                jsonText = Gdx.files.internal("level3EASY.json").readString();
            }
            else if (difficulty.equals("medium"))
            {
                jsonText = Gdx.files.internal("level3MEDIUM.json").readString();
            }
            else
            {
                jsonText = Gdx.files.internal("level3HARD.json").readString();
            }
    ...
    /**
     * Chooses the next word for the player to type and updates the game state accordingly.
     */
    ...
    /**
     * Renders the screen visuals and updates the game elements each frame.
     *
    ...
    public void render(float delta)
    {
        ...
        GlyphLayout titleLayout = new GlyphLayout(font, "LEVEL 3");
        ...
        font.setColor(0.8f, 0.6f, 0.4f, 1); // Set color to a visible one
        ...
        if (typingStarted && !allWordsTyped)
        {
            ...
            if (elapsedTime >= timer)
            {
                ...
                game.setScreen(new FailedScreen((SpellQuest) game, difficulty,3, score2));
            }
        }
        ...
        spriteBatch.draw(currentFrame2, offset + enemySprite.getWidth() + 370, Gdx.graphics.getHeight() / 2 - enemySprite.getHeight() / 3 - 10);
        spriteBatch.end();
        ...
        if (!allWordsTyped) {
            ...
            float score3 = getScore3();
            //System.out.println(score3);
            scorePrinted = true; // Set the flag so it doesn't print again

            // If the score is greater than 0, switch to GameScreenLevel3
            if ((score3-score2) > 0)
            {
                GameScreenLevel1.music.stop();
                ((Game) Gdx.app.getApplicationListener()).setScreen(new ResultsScreen(game,difficulty, getCurrScore(),getScore3(),0));
            }
        }
    }
    ...
    /**
     * Calculates the current score by adding the remaining time to the score brought from level 2.
     *
     * @return The total score accumulated across the levels.
     */
    public int getScore3()
    {
        if (difficulty.equals("easy"))
            return (int)(timer - elapsedTime) * 1000 + score2;
        else if (difficulty.equals("medium"))
            return (int)(timer - elapsedTime) * 1500+ score2;
        else
            return (int)(timer - elapsedTime) * 2500 + score2;
    ...
    public int getCurrScore()
    {
        if (difficulty.equals("easy"))
            ...
            return (int)(timer - elapsedTime) * 2000;
        else
            return (int)(timer - elapsedTime) * 3000;
