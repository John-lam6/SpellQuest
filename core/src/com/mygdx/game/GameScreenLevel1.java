...
/**
 * GameScreenLevel1 represents the first level of the game.
...
public class GameScreenLevel1 extends ScreenAdapter
{
    /** background music for the gameplay screens */
    public static Music music = Gdx.audio.newMusic (Gdx.files.internal("data/RetroPlatforming.mp3"));
    private Sound ding1;
    private Sound ding2;
    private Sound ding3;
    private Game game;
    ...
    /**
     * Constructs a new GameScreenLevel1 object.
     *
     * @param game The game object that controls the screens.
     * @param difficulty    The difficulty of the save.
     */
    public GameScreenLevel1(Game game, String difficulty)
    {
    ...
    public void show()
    {
        ...
        pauseButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                music.pause();
                count--;
                game.setScreen(new PauseScreen(game, GameScreenLevel1.this));
    ...
    /**
     * Loads the words from a JSON file and initializes the word list.
     */
    ...
    private void loadWords()
    {
        ...
        try
        {
            ...
            if (difficulty.equals("easy"))
            {
                jsonText = Gdx.files.internal("level1EASY.json").readString();
            }
            else if (difficulty.equals("medium"))
            {
                jsonText = Gdx.files.internal("level1MEDIUM.json").readString();
            }
            else
            {
                jsonText = Gdx.files.internal("level1HARD.json").readString();
            }
    ...
    /**
     * Picks the next word from the list of words for the player to type.
     */
    ...
    /**
     * The main render method called every frame, responsible for rendering the game's elements
    ...
    public void render(float delta)
    {
        ...
        GlyphLayout titleLayout = new GlyphLayout(font, "LEVEL 1");
        ...
        font.setColor(0.5f, 1, 0.7f, 1); // Set color to a visible one
        ...
        if (typingStarted && !allWordsTyped)
        {
            ...
            if (elapsedTime >= timer)
            {
                ...
                music.stop();
                game.setScreen(new FailedScreen((SpellQuest) game, difficulty, 1, 0));
            }
        }
        ...
        else if (!scorePrinted)
        {
            ...
            {
                music.stop();
                ((Game) Gdx.app.getApplicationListener()).setScreen(new ResultsScreen(game,difficulty, score,score,2));
            }
        }
    }
    /**
     * Calculates the score based on the remaining time and the difficulty.
     *
